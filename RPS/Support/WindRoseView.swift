...
/// The rose plus the numbers that go with it.
struct LegWindPanel: View {
    let legHeadingDeg: Double
    ...
    private var plan: SailPlan {
        SailPlan(legHeadingDeg: legHeadingDeg, windFromDeg: wind.fromDeg)
    }

    var body: some View {
        VStack(spacing: 12) {
            WindRoseView(legHeadingDeg: legHeadingDeg, windFromDeg: wind.fromDeg, size: 150)
            ...
            VStack(spacing: 4) {
                ...
                Text(plan.sails)
                    .font(.caption)
                    .foregroundStyle(plan.isBeat ? .orange : .secondary)
                    .multilineTextAlignment(.center)
            }

            // Always says where the number came from. This is model wind
            // from a forecast grid, not a masthead reading, and it can be
            // a long way out inshore or in a sea breeze.
            Text(
                "Forecast wind \(Int(wind.fromDeg.rounded()))° at \(Int(wind.speedKts.rounded())) kt"
